...
int main() {
    std::cout << "=== C++11 - Handle / Cancel / Timeout / Return Value ===" << std::endl << std::endl;
    ...
    /* Test 2: Cancel */
    std::cout << "[Test 2] Submit long task, cancel before execution" << std::endl;
    {
        auto h = pool.submit([] {
            std::cout << "  [should not run]" << std::endl;
            sleep_ms(2000);
        });
        /* don't sleep - try to cancel immediately while in queue */
        bool ok = h.cancel();
    ...
    std::cout << "[Test 3] Cancel a running task" << std::endl;
    {
        auto h = pool.submit([] {
            std::cout << "  [running, will be cancelled mid-flight]" << std::endl;
            sleep_ms(2000);
            std::cout << "  [finished anyway]" << std::endl;
        });
        sleep_ms(100);
        bool ok = h.cancel();
        std::cout << "  cancel() returned " << ok << " (task already running)" << std::endl;
        h.wait_for(std::chrono::seconds(5));
